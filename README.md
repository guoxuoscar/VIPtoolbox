...
| **文件工具** | 文件夹批量创建（按 Excel 列层级嵌套）、文件提取汇总（按文件夹名匹配提取，可自定输出结构和命名规则）、图片归入同名文件夹、批量分发文件、供应商款号提取、**批量合并表格**（5种合并模式：纵向按位置/按列名、横向VLOOKUP、多表合一、高级合并，支持来源标记、去重、Sheet选择、CSV）、导出文件清单、清理重复文件 |
| **操作历史** | 记录所有操作日志，方便回溯 |
...
└── toolbox/
    ...
    │   ├── file_tools_page.py    # 文件工具（建文件夹/提取汇总/图片归入/分发/供应商款号/合并表格/清单导出/清重）
